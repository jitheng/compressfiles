...
import {
  trackImageFileUploaded,
  trackImageCompressionStarted,
  trackImageCompressionSuccess,
  trackImageDownloadClicked,
...
export default function ImageCompressor() {
  ...
  const handleFile = useCallback(
    (f) => {
      setFile(f)
      reset()
      trackImageFileUploaded({ file_size_bytes: f.size, file_name: f.name, file_type: f.type })
    },
    [reset],
  )
  ...
  const handleCompress = useCallback(() => {
    if (!file) return
    trackImageCompressionStarted({ compression_level: level, file_size_bytes: file.size })
    compress(file, level)
  }, [file, level, compress])
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      {file && !isDone && (
        <button
          ...
          disabled={isBusy}
          data-testid="compress-image-btn"
        >
          {isBusy ? (
            <>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 space-y-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Your image is ready to download</p>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            ...
            */}
            ...
            <button
              ...
              onClick={handleReset}
              data-testid="compress-another-image-btn"
            >
              Compress another
            </button>
          </div>
